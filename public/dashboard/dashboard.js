...
btnFanfare.addEventListener('click', async () => {
  ...
  try { const res = await fetch('/announce/fanfare', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ file: FANFARE_FILE }) }); if (!res.ok) throw new Error('HTTP ' + res.status); btnFanfare.className = 'btn-fanfare state-ok'; btnFanfareLabel.textContent = '\u2713 Gestartet'; } catch (_) { btnFanfare.className = 'btn-fanfare state-err'; btnFanfareLabel.textContent = '\u2717 Fehler'; btnFanfare.disabled = false; }
...
function connect() { ws = new WebSocket(WS_URL); ws.addEventListener('open', () => { setStatus('connected'); reconnectDelay = RECONNECT_BASE; clearTimeout(reconnectTimer); }); ws.addEventListener('message', ({ data }) => { try { handleMessage(JSON.parse(data)); } catch (_) {} }); ws.addEventListener('close', () => { setStatus('reconnecting'); scheduleReconnect(); }); ws.addEventListener('error', () => { ws.close(); }); }
...
function handleMessage(msg) { switch (msg.type) { case 'snapshot': applySnapshot(msg); break; case 'speech': applySpeech(msg.payload); break; case 'speech:clear': clearSpeech(); break; case 'queue': applyQueue(msg.payload); break; case 'history': applyHistory(msg.payload); break; case 'error': applyErrors(msg.payload); break; case 'server': applyServer(msg.payload); break; } }
function applySnapshot(snap) { if (snap.server) applyServer(snap.server); if (snap.currentSpeech) applySpeech(snap.currentSpeech); else clearSpeech(); if (snap.queue) applyQueue(snap.queue); if (snap.history) applyHistory(snap.history); if (snap.errors) applyErrors(snap.errors); }
